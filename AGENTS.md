...
Update `AGENTS.md` for the current UstoHub Lite project state.

Task type:
Documentation/rules update only.
...
Goal:
The project now officially supports a multilingual frontend using Django i18n.
...
1. Replace the old English-only language rule.

Remove or rewrite the rule that says:

* current frontend must use English only
* multilingual functionality is not allowed
* language switchers are not allowed
* translation files are not allowed
* Django i18n is not allowed

New language rule:

* UstoHub Lite supports English, Russian, and Tajik through Django i18n.
...
* Do not create duplicated templates like `home_en.html`, `home_ru.html`, or `home_tg.html`.
* Do not translate backend identifiers, URL names, CSS classes, template block names, form field names, model values, or status values.
* Backend values such as `customer`, `master`, `open`, `in_progress`, `completed`, and `cancelled` must remain unchanged.
* Visible labels may be translated for users, but submitted backend values must stay unchanged.

2. Update protected backend file rules with a narrow i18n exception.

Normally Codex must not edit `settings.py` or root `urls.py`.

Exception:
For an explicit multilingual/i18n task, Codex may edit only minimal Django i18n configuration:
...
3. Update forbidden project changes.

Replace the outdated rule “do not add multilingual systems” with:

* Do not add a multilingual system outside Django i18n.
* Do not replace Django i18n with JavaScript-only translation.
* Do not add duplicated language templates.
* Do not add external translation APIs.
* Do not translate backend data values.
* Do not fake translations with hardcoded frontend conditionals.

4. Add language switcher rules.
...
* Show EN / RU / TG clearly.
* The active language must be visually clear.
* The switcher must work in light and dark mode.
* The switcher must not break navbar layout.
* The switcher must be responsive and accessible.
* Do not redesign the whole navbar unless explicitly requested.

5. Add translation file rules.
...
6. Keep all frontend design rules.

Do not weaken:

* Industrial Premium Marketplace direction
...
7. Update final report requirements.

For multilingual/i18n tasks, Codex must report:
...
Final report must clearly confirm:

* No backend business logic was changed.
* No migrations were created or run.
* No database changes were made.
* No packages were installed unless explicitly permitted.
* No commits or pushes were performed.

Important:
Only update `AGENTS.md` in this task. Do not implement new frontend features yet.
